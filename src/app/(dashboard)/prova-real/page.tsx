...
export default function ProvaRealPage() {
  ...
  async function generatePlan() {
    ...
    setGenerating(true); setProgress(8); setPlan(null); setAnswers({}); setCorrected(false)
    try {
      ...
      setProgress(100); setProgressStep('Simulado clicável gerado com sucesso'); setPlan(data.plan); setCreditsRemaining(typeof data.creditsRemaining === 'number' ? data.creditsRemaining : null)
      toast.success('Simulado clicável gerado com sucesso.')
    } catch (e) {
      setProgress(0); setProgressStep(PROGRESS_STEPS[0]); toast.error((e as Error).message || 'Erro ao gerar.')
        ...
        <div className="space-y-4">
          <div className="card p-5 space-y-4">
            ...
            <div className="grid md:grid-cols-2 gap-3">
              <div><label className="label">Banca</label><input className="input" value={banca} onChange={e => setBanca(e.target.value)} placeholder="Ex: FURB, FGV, Cebraspe" /></div>
              <div><label className="label">Cargo</label><input className="input" value={cargo} onChange={e => setCargo(e.target.value)} placeholder="Ex: Professor" /></div>
            </div>
          ...
          <div className="card p-5 space-y-4">
            ...
            <div className="rounded-2xl border-2 border-dashed border-white/10 bg-zinc-900/50 p-5 text-center hover:border-brand-500/40 transition-colors cursor-pointer" onClick={() => document.getElementById('prova-file')?.click()}>
              <Upload size={28} className="mx-auto mb-3 text-brand-300" /><div className="font-heading font-bold text-white">Enviar prova real</div>
              <p className="text-xs text-zinc-500 mt-1">PDF pesquisável ou TXT. {provaName ? `Arquivo carregado: ${provaName}` : 'Nenhuma prova carregada ainda.'}</p>
            ...
            <div><label className="label">Ou cole o texto da prova real</label><textarea className="input min-h-[130px] resize-y" value={provaText} onChange={e => setProvaText(e.target.value)} placeholder="Cole aqui as questões reais da prova..." /><div className="text-[11px] text-zinc-500 mt-1">Texto atual: {provaText.length.toLocaleString('pt-BR')} caracteres</div></div>
          </div>

          <div className="card p-5 space-y-4">
            <div className="rounded-xl border border-green-500/20 bg-green-500/10 p-3 text-xs text-green-100">PASSO 3: envie ou cole o gabarito oficial. A correção depende dele.</div>
            <div className="rounded-2xl border-2 border-dashed border-white/10 bg-zinc-900/50 p-5 text-center hover:border-green-500/40 transition-colors cursor-pointer" onClick={() => document.getElementById('gabarito-file')?.click()}>
              <FileText size={28} className="mx-auto mb-3 text-green-300" /><div className="font-heading font-bold text-white">Enviar gabarito oficial</div>
              <p className="text-xs text-zinc-500 mt-1">PDF ou TXT. {gabaritoName ? `Arquivo carregado: ${gabaritoName}` : 'Nenhum gabarito carregado ainda.'}</p>
            ...
            <div><label className="label">Ou cole o gabarito oficial</label><textarea className="input min-h-[110px] resize-y" value={gabaritoText} onChange={e => setGabaritoText(e.target.value)} placeholder="Ex: 1-B, 2-C, 3-E..." /></div>
          </div>
