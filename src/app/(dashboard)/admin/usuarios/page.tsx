...
const PLANS = [
  ...
  { value: 'CADERNOS_500', label: 'Cadernos 500' },
  { value: 'PRO', label: 'Pro' },
  ...
  async function saveUser(user: UserRow) {
    ...
    try {
      const res = await fetch('/api/admin/users', {
        ...
        body: JSON.stringify({ userId: user.id, plan, role, credits }),
      })
      const data = await res.json()
      if (!res.ok) {
  ...
  return (
    <div className="p-6 max-w-6xl mx-auto">
      <div className="mb-6">
        ...
        <p className="text-zinc-400 text-sm mt-1">Busque usuários e altere plano, créditos e permissão manualmente.</p>
      </div>
                    ...
                    <tr key={user.id} className="border-b border-white/[0.04] hover:bg-white/[0.02]">
                      ...
                      <td className="px-4 py-3">
                        <select
                          ...
                          value={String(edit.role ?? user.role)}
                      ...
                      <td className="px-4 py-3">
                        <select
                          className="bg-zinc-800 border border-white/10 rounded-lg px-2 py-1 text-xs text-zinc-300 outline-none"
                          value={String(edit.plan ?? user.plan)}
                          onChange={e => updateEdit(user.id, 'plan', e.target.value)}
                        >
                      ...
                      <td className="px-4 py-3">
                        <input
                          type="number"
                          min={0}
                          className="w-24 bg-zinc-800 border border-white/10 rounded-lg px-2 py-1 text-xs text-zinc-300 outline-none"
                          value={Number(edit.credits ?? user.credits)}
                          onChange={e => updateEdit(user.id, 'credits', Number(e.target.value))}
                        />
                      </td>
                      <td className="px-4 py-3">
                        <button
                          ...
                          className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-brand-600 hover:bg-brand-500 text-white text-xs font-semibold disabled:opacity-50"
                        >
