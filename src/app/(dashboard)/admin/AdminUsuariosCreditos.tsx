...
export default function AdminUsuariosCreditos() {
  ...
  async function updateUser(userId: string, payload: Record<string, string | number>) {
    setSaving(userId)
    try {
      const res = await fetch('/api/admin/stats', {
        method: 'POST',
      ...
      <div className="card overflow-hidden">
        <div className="p-4 border-b border-white/[0.07] flex items-center justify-between gap-3">
          <div>
            ...
            <p className="text-xs text-zinc-500 mt-1">Ajuste créditos, role e plano de cada usuário.</p>
          </div>
