...
const BANCAS = ['INSTITUTO AOCP', 'AVANÇA SP', 'CEBRASPE', 'CONSULPLAN', 'FUNDATEC', 'OBJETIVA', 'QUADRIX', 'VUNESP', 'FEPESE', 'FAURGS', 'FAFIPA', 'IDECAN', 'CESPE', 'AOCP', 'IBFC', 'FURB', 'FGV', 'FCC']
...
function findKnownBancas(texto: string) {
  const normalized = normalizeSearch(texto)
  const found: string[] = []
  for (const banca of BANCAS) {
    const key = normalizeSearch(banca)
    const pattern = new RegExp(`(?:^|[^A-Z0-9])${escapeRegex(key)}(?:[^A-Z0-9]|$)`, 'i')
    if (pattern.test(normalized) && !found.some(existing => normalizeSearch(existing).includes(key) || key.includes(normalizeSearch(existing)))) found.push(banca)
  }
  return found
}
function detectarBanca(texto: string) {
  const linhas = texto.split(/\n|\r/).map(l => l.trim()).filter(Boolean)
  const termos = ['BANCA', 'BANCA ORGANIZADORA', 'ORGANIZADORA', 'EMPRESA ORGANIZADORA', 'INSTITUICAO ORGANIZADORA', 'INSTITUIÇÃO ORGANIZADORA', 'INSTITUTO ORGANIZADOR', 'EXECUTORA', 'EMPRESA EXECUTORA', 'RESPONSAVEL PELA ORGANIZACAO', 'RESPONSÁVEL PELA ORGANIZAÇÃO', 'SOB RESPONSABILIDADE', 'A CARGO', 'REALIZACAO', 'REALIZAÇÃO']
  for (const linha of linhas) {
    const n = normalizeSearch(linha)
    if (!termos.some(t => n.includes(normalizeSearch(t)))) continue
    const found = findKnownBancas(linha)
    if (found.length) return found[0]
  }
  const all = findKnownBancas(texto)
...
export default function GerarPage() {
  ...
  const [lastBatchQuantity, setLastBatchQuantity] = useState(0)
  ...
  function aplicarTextoDoEdital(raw: string) {
    ...
    else if (!banca.trim()) toast.info('Não consegui identificar a banca com segurança. Confira ou preencha manualmente.')
  ...
  async function gerar(append = false) {
    ...
    const batchQuantity = append ? (lastBatchQuantity || quantity) : quantity
    generateLockRef.current = true
    setLoading(true)
    if (!append) { setQuestions([]); setAnswered({}) }
    setLastError(null)
    try {
      ...
      const payload = { banca, area, cargo, city, uf, education, difficulty, type, format, quantity: batchQuantity, provider, editalText: contextoEdital || undefined }
      ...
      const novas = data.questions || []
      setQuestions(prev => append ? [...prev, ...novas] : novas)
      setLastBatchQuantity(batchQuantity)
      toast.success(append ? `Mais ${novas.length} questão(ões) adicionada(s)!` : `${novas.length} questão(ões) gerada(s)!`)
    } catch (e) { handleError('Erro ao gerar questão', 'generate_question_exception', { error: (e as Error).message, banca, area, cargo, city, uf, quantity: batchQuantity }) }
  ...
  async function responder(q: Question, idx: number) {
    ...
    try {
      const res = await fetch('/api/ai/answer', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ questionId: q.id, selectedIdx: idx }) })
      const data = await res.json().catch(() => ({}))
      if (!res.ok) { toast.error(data.error || 'Não foi possível registrar a resposta'); return }
  ...
  return (
    <div className="p-4 md:p-6 max-w-6xl mx-auto">
      <div className="mb-8"><h1 className="font-heading text-2xl font-bold">✦ Gerar Questão</h1><p className="text-zinc-400 text-sm mt-1">A IA cria questões no estilo da banca com origem, prova, ano e base de conteúdo.</p></div>
      <div className="mb-6 rounded-2xl border border-amber-500/20 bg-amber-500/10 p-4 text-sm text-amber-100"><strong>Quanto mais informações você preencher, melhor fica a questão.</strong> Banca e área são obrigatórias. Cargo, cidade, UF, ano e edital são opcionais, mas deixam a questão mais próxima da prova real.</div>
      <div className="grid lg:grid-cols-[420px_1fr] gap-6">
        <div className="space-y-4">
          <div className="card p-5"><div className="font-heading text-sm font-semibold text-brand-300 mb-4">1. Banca <RequiredBadge /></div><input className="input" placeholder="Ex: CEBRASPE, FCC, FGV, FEPESE, FAURGS..." value={banca} onChange={e => setBanca(e.target.value)} /><p className="text-xs text-zinc-600 mt-2">Obrigatório. Define o estilo da cobrança, pegadinhas e linguagem da questão.</p></div>
          <div className="card p-5"><div className="font-heading text-sm font-semibold text-brand-300 mb-4">2. Área do conhecimento <RequiredBadge /></div><input className="input" placeholder="Ex: Direito Constitucional, Matemática, Informática..." value={area} onChange={e => setArea(e.target.value)} /><p className="text-xs text-zinc-600 mt-2">Obrigatório. Limita o conteúdo técnico que a IA pode cobrar.</p></div>
          <div className="card p-5 space-y-4"><div className="font-heading text-sm font-semibold text-brand-300">3. Contexto do edital <OptionalBadge /></div><div><label className="label">Referência do edital/concurso <OptionalBadge /></label><input className="input" placeholder="Ex: Prefeitura de Florianópolis — Administrativo" value={editalRef} onChange={e => setEditalRef(e.target.value)} /><p className="text-xs text-zinc-600 mt-2">Ajuda a montar a origem da questão: órgão, prova, cargo e contexto.</p></div><div className="grid grid-cols-2 gap-3"><div><label className="label">Ano <OptionalBadge /></label><input className="input" placeholder="2026" value={editalYear} onChange={e => setEditalYear(e.target.value.replace(/\D/g, '').slice(0, 4))} /></div><div><label className="label">UF <OptionalBadge /></label><select className="input" value={uf} onChange={e => setUf(e.target.value)}><option value="">Selecione</option>{UFS.map(item => <option key={item} value={item}>{item}</option>)}</select></div></div><div><label className="label">Cidade/Município <OptionalBadge /></label><input className="input" placeholder="Ex: Florianópolis" value={city} onChange={e => setCity(e.target.value)} /></div>{isAdmin ? <div className="rounded-xl border border-white/10 bg-black/20 p-3 space-y-3"><label className="flex items-center gap-2 text-sm cursor-pointer"><input type="checkbox" checked={useWebSearch} onChange={e => setUseWebSearch(e.target.checked)} /><span>Buscar referências públicas na web com SerpAPI</span></label>{useWebSearch && <div className="space-y-2"><input className="input" placeholder="Ex: edital Florianópolis 2026 banca cargo" value={webQuery} onChange={e => setWebQuery(e.target.value)} /><button type="button" onClick={buscarWebContexto} disabled={searchingWeb} className="btn-secondary text-xs flex items-center gap-1">{searchingWeb ? <Loader2 size={14} className="animate-spin" /> : <Search size={14} />} Buscar agora</button>{webResults.length > 0 && <div className="space-y-1 max-h-40 overflow-y-auto">{webResults.map((r, i) => <div key={i} className="text-[11px] text-zinc-500 border border-white/5 rounded-lg p-2"><div className="text-zinc-300 font-medium">{r.title}</div><div>{r.snippet}</div></div>)}</div>}</div>}</div> : <div className="rounded-xl border border-zinc-700/60 bg-zinc-900/60 p-3 text-xs text-zinc-400"><div className="flex items-center justify-between gap-3"><span>Buscar referências públicas na web com SerpAPI</span><span className="rounded-full bg-zinc-800 px-2 py-1 text-[10px] font-semibold text-zinc-300">Em breve</span></div></div>}<div className={`border-2 border-dashed rounded-2xl p-6 text-center cursor-pointer transition-all ${dragging ? 'border-brand-500 bg-brand-500/5' : 'border-white/10 hover:border-white/20'}`} onClick={() => document.getElementById('gerar-edital-file')?.click()} onDragOver={e => { e.preventDefault(); setDragging(true) }} onDragLeave={() => setDragging(false)} onDrop={e => { e.preventDefault(); setDragging(false); const f = e.dataTransfer.files[0]; if (f) processEditalFile(f) }}><Upload size={24} className="mx-auto mb-2 text-zinc-500" /><div className="font-heading font-semibold text-sm mb-1">{editalFileName || 'Enviar edital original (opcional)'}</div><div className="text-xs text-zinc-500">PDF ou TXT — tenta reconhecer banca, ano, UF, cidade e cargos com segurança</div>{editalText && <div className="mt-2 text-xs text-green-400">✓ {Math.round(editalText.length / 100) / 10}kb de contexto carregado</div>}</div><input type="file" id="gerar-edital-file" accept=".pdf,.txt,.doc,.docx" className="hidden" onChange={e => { const f = e.target.files?.[0]; if (f) processEditalFile(f) }} />{editalText && <button type="button" className="btn-secondary text-xs" onClick={() => { setEditalText(''); setEditalFileName(''); setDetectedCargos([]) }}>Remover edital carregado</button>}{detectedCargos.length > 0 && <div className="rounded-xl border border-brand-500/20 bg-brand-500/5 p-3"><div className="text-xs font-semibold text-brand-300 mb-2">Cargos encontrados no edital</div><div className="flex flex-wrap gap-2">{detectedCargos.map(c => <button key={c} type="button" onClick={() => setCargo(c)} className="chip text-xs">{c}</button>)}</div></div>}</div>
          <div className="card p-5 space-y-4"><div className="font-heading text-sm font-semibold text-brand-300">4. Configurações <OptionalBadge /></div><div><label className="label">Cargo pretendido <OptionalBadge /></label><input className="input" placeholder="Ex: Agente Administrativo, Analista..." value={cargo} onChange={e => setCargo(e.target.value)} /><p className="text-xs text-zinc-600 mt-2">Opcional, mas recomendado. Ajusta o nível e o contexto da pergunta.</p></div><div><label className="label">Nível de escolaridade <OptionalBadge /></label><div className="flex flex-wrap gap-2">{ESCOLARIDADES.map(e => <button key={e} onClick={() => setEducation(e)} className={`chip text-xs ${education === e ? 'chip-active' : ''}`}>{e}</button>)}</div></div><div><label className="label">Dificuldade</label><div className="flex gap-2">{DIFS.map(d => <button key={d} onClick={() => setDifficulty(d)} className={`chip ${difficulty === d ? 'chip-active' : ''}`}>{d}</button>)}</div></div><div><label className="label">Tipo de questão</label><div className="flex gap-2">{TIPOS.map(t => <button key={t} onClick={() => setType(t)} className={`chip ${type === t ? 'chip-active' : ''}`}>{TIPO_LABELS[t]}</button>)}</div></div><div><label className="label">Formato</label><div className="flex gap-2">{FMTS.map(f => <button key={f} onClick={() => setFormat(f)} className={`chip ${format === f ? 'chip-active' : ''}`}>{f}</button>)}</div></div>{isAdmin && <div><label className="label">Provedor de IA</label><div className="flex flex-wrap gap-2">{availableProviders.map(p => { const labels: Record<string,string> = {claude:'🟠 Claude',openai:'🟢 ChatGPT',gemini:'🔵 Gemini',grok:'⚡ Grok',openrouter:'🔶 OpenRouter'}; return <button key={p} onClick={() => setProvider(p)} className={`chip ${provider === p ? 'chip-active' : ''}`}>{labels[p] || p}</button> })}</div></div>}<div><label className="label">Quantidade ({maxQtd} máx.)</label><div className="flex flex-wrap gap-2">{Array.from({ length: maxQtd }, (_, i) => i + 1).map(n => <button key={n} onClick={() => setQuantity(n)} className={`w-9 h-9 rounded-xl border text-sm font-semibold transition-all ${quantity === n ? 'bg-brand-600 border-brand-500 text-white' : 'border-white/10 text-zinc-400 hover:border-brand-500'}`}>{n}</button>)}</div></div></div>
          <button onClick={() => gerar(false)} disabled={loading || searchingWeb || generateLockRef.current} className="w-full bg-gradient-to-r from-brand-600 to-purple-600 text-white font-semibold rounded-xl px-6 py-3.5 flex items-center justify-center gap-2 transition-all hover:opacity-90 disabled:opacity-40">{loading ? <><Loader2 size={16} className="animate-spin" />Gerando...</> : searchingWeb ? <><Loader2 size={16} className="animate-spin" />Buscando...</> : '✦ Gerar questões com IA'}</button>
        </div>
        <div>
          {lastError && <div className="card p-4 mb-4 border-red-500/30 bg-red-500/10"><div className="flex items-start gap-3"><AlertTriangle size={18} className="text-red-300 mt-0.5 shrink-0" /><div className="flex-1"><div className="font-semibold text-red-200 text-sm">Ocorreu um erro</div><div className="text-xs text-red-100/80 mt-1">{lastError.message}</div><div className="flex flex-wrap gap-2 mt-3"><button type="button" onClick={() => gerar(false)} className="btn-secondary text-xs">Tentar novamente</button><button type="button" onClick={reportarErro} disabled={reportingError} className="rounded-xl border border-red-400/30 bg-red-500/10 px-3 py-2 text-xs font-semibold text-red-100 disabled:opacity-50">{reportingError ? 'Reportando...' : 'Reportar erro'}</button></div></div></div></div>}
          {loading && <LoadingProgress title="Gerando questões..." description="A IA está analisando o contexto, criando alternativas e preparando os comentários." />}
          ...
          {questions.length > 0 && <div className="card p-5 text-center border-brand-500/20 bg-brand-500/5"><div className="text-sm font-semibold text-zinc-100 mb-1">Quer continuar com o mesmo contexto?</div><div className="text-xs text-zinc-500 mb-4">Adiciona novas questões no final sem apagar as atuais.</div><button onClick={() => gerar(true)} disabled={loading || searchingWeb || generateLockRef.current} className="btn-primary inline-flex items-center justify-center gap-2 px-5 py-3">{loading ? <><Loader2 size={15} className="animate-spin" />Gerando...</> : `Gerar mais ${lastBatchQuantity || quantity} questões`}</button></div>}
        </div>
      </div>
    </div>
  )
