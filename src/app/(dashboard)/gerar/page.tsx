...
export default function GerarPage() {
  ...
  async function responder(q: Question, idx: number) { if (answered[q.id] !== undefined) return; setAnswered(p => ({ ...p, [q.id]: idx })) }
        ...
        <div className="space-y-4">
          ...
          <div className="card p-5 space-y-4">
            ...
            <div className="grid grid-cols-2 gap-3">
              <div><label className="label">Ano <OptionalBadge /></label><input className="input" placeholder="2026" value={editalYear} onChange={e => setEditalYear(e.target.value.replace(/\D/g, '').slice(0, 4))} /></div>
              <div><label className="label">UF <OptionalBadge /></label><select className="input" value={uf} onChange={e => setUf(e.target.value)}><option value="">Selecione</option>{UFS.map(item => <option key={item} value={item}>{item}</option>)}</select></div>
            </div>
