...
import { Loader2, FileDown, FileText, HelpCircle, Trash2, Search, SlidersHorizontal, BarChart3, RotateCcw } from 'lucide-react'
...
export default function GeradosPage() {
  ...
  const [tab, setTab] = useState<'planos' | 'questoes'>('questoes')
  ...
  async function clearAllQuestions() {
    if (!questions.length) return toast.info('Não há questões para limpar')
    if (!confirm('Tem certeza que deseja limpar TODAS as questões geradas? Essa ação remove a lista de questões em Meus Gerados.')) return
    setClearing(true)
    try {
      const res = await fetch('/api/generated', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ action: 'delete_all_questions' }),
      })
      const data = await res.json().catch(() => ({}))
  ...
  async function exportQuestionsPDF() {
    ...
    addLine('GabaritoIA - Questões para responder', 16, true)
    ...
    doc.save('gabaritoia-questoes-para-responder.pdf')
  }
      ...
      {tab === 'questoes' && (
        <div>
          <div className="flex flex-wrap justify-end gap-2 mb-4">
            <button onClick={load} className="btn-secondary text-xs flex items-center gap-1"><RotateCcw size={14} /> Atualizar</button>
            ...
            <button onClick={clearAllQuestions} disabled={!questions.length || clearing} className="rounded-xl border border-red-500/20 bg-red-500/10 text-red-300 px-3 py-2 text-xs flex items-center gap-1 disabled:opacity-50">
              {clearing ? <Loader2 size={14} className="animate-spin" /> : <Trash2 size={14} />} Limpar tudo
            </button>
          </div>
          {questions.length === 0 && <div className="card p-8 text-center text-zinc-500">Nenhuma questão gerada salva ainda. Gere questões pelo Gerador ou pelo Plano de Questões.</div>}
              ...
              <div key={plan.id} className="card p-5">
                <div className="flex flex-col md:flex-row md:items-start justify-between gap-3">
                  ...
                  <div className="flex flex-wrap gap-2">
                    ...
                    <button className="btn-secondary text-xs" onClick={() => setOpenPlan(isOpen ? null : plan.id)}>{isOpen ? 'Fechar' : 'Detalhes'}</button>
