...
type ImportSummary = { title: string; totalQuestions: number; expectedQuestions?: number; alreadyImported?: boolean; parser?: string }
type PageInfo = { limit: number; offset: number; returned: number; hasMore: boolean }
...
function importCountLabel(summary: ImportSummary) {
  if (summary.expectedQuestions && summary.expectedQuestions > summary.totalQuestions) return `${summary.totalQuestions} de ${summary.expectedQuestions} questões reconhecidas`
...
function ImportProgressBox({ percent, stage, seconds }: { percent: number; stage: string; seconds: number }) {
  return (
    <div className="mb-6 rounded-3xl border border-brand-500/20 bg-brand-500/10 p-5">
      ...
      <div className="mt-3 text-[11px] text-zinc-500">Tempo decorrido: {seconds}s · Extraindo texto e organizando questões.</div>
      {seconds > 75 && <div className="mt-3 rounded-2xl border border-amber-500/20 bg-amber-500/10 p-3 text-xs text-amber-100">PDF grande detectado. Este modelo tem muitas páginas, então a primeira importação pode demorar, principalmente no celular.</div>}
    </div>
  )
}
...
export default function CadernosPage() {
  ...
  const [pageInfo, setPageInfo] = useState<PageInfo>({ limit: PAGE_SIZE, offset: 0, returned: 0, hasMore: false })
  ...
  useEffect(() => {
    ...
    const timer = window.setInterval(() => {
      ...
      setImportProgress(prev => Math.min(98, prev < 92 ? prev + 1 : prev + 0.25))
    }, 2500)
    return () => window.clearInterval(timer)
  }, [importing])
  ...
  async function fetchBookPage(book: Book, offset: number, append: boolean) {
    if (append) setLoadingMore(true)
    else setLoadingBook(true)

    try {
      const res = await fetch(`/api/question-books/${book.id}?limit=${PAGE_SIZE}&offset=${offset}`)
      const data = await readJsonSafe(res)
      ...
      if (append) {
        setQuestions(prev => {
          ...
          return [...prev, ...incoming.filter((q: Question) => !seen.has(q.id))]
        })
      } else {
      ...
      setActiveBook(data.book || book)
      setStats(data.stats || { answered: 0, correct: 0, total: incoming.length })
      setPageInfo(data.page || { limit: PAGE_SIZE, offset, returned: incoming.length, hasMore: false })
    } finally {
  ...
  async function openBook(book: Book) {
    ...
    setCurrent(0)
    setQuestions([])
    setPageInfo({ limit: PAGE_SIZE, offset: 0, returned: 0, hasMore: false })
    setFilterBanca('')
    setFilterYear('')
    setFilterUf('')
    setSearch('')
    await fetchBookPage(book, 0, false)
  }

  async function loadMoreQuestions() {
    if (!activeBook || loadingMore) return
    await fetchBookPage(activeBook, questions.length, true)
  }

  async function checkAlreadyImported(fileHash: string, fileName: string) {
  ...
  async function extractPdfText(buffer: ArrayBuffer) {
    ...
    for (let i = 1; i <= totalPages; i++) {
      ...
      setImportProgress(Math.min(74, 12 + Math.round((i / totalPages) * 62)))
      const page = await pdf.getPage(i)
      const content = await page.getTextContent()
      const pageText = content.items.map((x: any) => x.str || '').join(' ')
      if (pageText) parts.push(`--- PÁGINA ${i} ---\n${pageText}`)
      if (i % 5 === 0) await new Promise(resolve => window.setTimeout(resolve, 0))
    }
  ...
  async function importFile(file: File) {
    ...
    try {
      ...
      const existing = await checkAlreadyImported(fileHash, file.name)
      if (existing) return
      ...
      setImportProgress(84)
      serverProcessingRef.current = true
      const { res, data } = await postJson('/api/question-books', { action: 'import', title: file.name.replace(/\.pdf$/i, ''), text, fileHash })
      serverProcessingRef.current = false
      ...
      if (data.alreadyImported) toast.info(`Este caderno já foi importado. ${label}.`)
  ...
  async function answer(q: Question, idx: number) {
    ...
    setAnswering(q.id)
    try {
      const { res, data } = await postJson(`/api/question-books/${activeBook.id}`, { action: 'answer', questionId: q.id, selectedIndex: idx })
  ...
  async function deleteBook(book: Book) {
    ...
    const { res } = await postJson('/api/question-books', { action: 'delete', id: book.id })
  ...
  return (
    <div className="p-4 md:p-6 max-w-7xl mx-auto pb-36 md:pb-6">
