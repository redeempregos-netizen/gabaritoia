...
export default function CadernosPage() {
  ...
  async function importFile(file: File) {
    ...
    try {
      ...
      toast.success(`${data.book.totalQuestions} questões importadas!`)
      await loadBooks()
    } catch (e) {
  ...
  async function answer(q: Question, idx: number) {
    ...
    try {
      ...
      setStats(prev => ({
        ...prev,
        answered: wasAnswered ? prev.answered : prev.answered + 1,
        correct: prev.correct + (data.isCorrect ? 1 : 0) - (q.isCorrect ? 1 : 0),
      }))
  ...
  async function deleteBook(book: Book) {
    ...
    const res = await fetch('/api/question-books', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ action: 'delete', id: book.id }),
    })
    ...
    <div className="p-4 md:p-6 max-w-7xl mx-auto">
      <div className="mb-6 rounded-3xl border border-brand-500/20 bg-gradient-to-br from-brand-500/10 via-zinc-900 to-zinc-950 p-5 md:p-7">
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
          <div>
            <div className="inline-flex items-center gap-2 rounded-full border border-brand-500/20 bg-brand-500/10 px-3 py-1 text-xs text-brand-200 mb-3"><BookOpen size={13} /> Novo recurso</div>
            <h1 className="font-heading text-2xl md:text-3xl font-bold">Cadernos de Questões PDF</h1>
            <p className="text-zinc-400 text-sm mt-2 max-w-2xl">Importe PDFs de questões comentadas, extraia gabarito e comentários automaticamente e responda dentro da plataforma.</p>
          </div>
      ...
      {loading ? <div className="p-8 flex justify-center"><Loader2 className="animate-spin text-brand-400" /></div> : (
        <div className="grid lg:grid-cols-[340px_1fr] gap-6">
          ...
          <div>
            {!activeBook || !q ? <div className="card p-12 text-center text-zinc-500">Selecione um caderno importado ou envie um PDF para começar.</div> : <div className="space-y-5">
              <div className="rounded-3xl border border-white/[0.08] bg-gradient-to-br from-zinc-900 via-zinc-900 to-brand-950/30 p-5">
                <div className="flex flex-col md:flex-row md:items-start justify-between gap-4">
                  ...
                  <div className="grid grid-cols-3 gap-2 text-center min-w-full md:min-w-[260px]">
                    ...
                    <div className="rounded-2xl bg-black/25 border border-white/10 p-3"><div className="font-bold text-brand-300">{current + 1}/{questions.length}</div><div className="text-[10px] text-zinc-500">Atual</div></div>
                  </div>
                </div>
              </div>
              ...
              <div className="flex justify-between gap-3">
                ...
                <button className="btn-primary" disabled={current >= questions.length - 1} onClick={() => setCurrent(c => Math.min(questions.length - 1, c + 1))}>Próxima</button>
              </div>
            </div>}
          </div>
        </div>
