...
import { acquireAISlot, releaseAISlot } from './aiQueue'
...
async function getApiKey(provider: AIProvider): Promise<{ key: string; model: string } | null> {
  try {
    const record = await prisma.apiKey.findUnique({ where: { provider, isEnabled: true } })
...
export async function callAI({ prompt, provider = 'claude', maxTokens = 2000, systemPrompt, useCache = true, cacheTTL, action = 'unknown', queueJobId }: AICallOptions): Promise<string> {
  ...
  try {
    jobId = await acquireAISlot(action, provider, queueJobId)
...
interface CallProviderOptions { provider: AIProvider; prompt: string; systemPrompt?: string; maxTokens: number; apiKey?: string | null; model: string }

async function callProvider({ provider, prompt, systemPrompt, maxTokens, apiKey, model }: CallProviderOptions): Promise<string> {
  if (!apiKey && provider !== 'claude') throw new Error(`Chave de API para ${provider} não configurada. Configure no painel Admin.`)
  switch (provider) {
    case 'claude': return callClaude(prompt, systemPrompt, maxTokens, apiKey, model)
    case 'openai': return callOpenAI(prompt, systemPrompt, maxTokens, apiKey!, model)
    case 'gemini': return callGemini(prompt, maxTokens, apiKey!, model)
    case 'grok': return callOpenAICompat(prompt, systemPrompt, maxTokens, apiKey!, model, 'https://api.x.ai/v1')
    case 'openrouter': return callOpenRouter(prompt, systemPrompt, maxTokens, apiKey!, model)
...
async function callClaude(prompt: string, system: string | undefined, maxTokens: number, apiKey?: string | null, model?: string): Promise<string> {
  ...
  const msg = await client.messages.create({ model: model || 'claude-sonnet-4-20250514', max_tokens: maxTokens, ...(system && { system }), messages: [{ role: 'user', content: prompt }] })
  return msg.content.map(b => b.type === 'text' ? b.text : '').join('')
}

function isGPT5Model(model: string): boolean { return model.toLowerCase().startsWith('gpt-5') }
...
async function callOpenAI(prompt: string, system: string | undefined, maxTokens: number, apiKey: string, model: string): Promise<string> {
  if (isGPT5Model(model)) {
    const res = await fetch('https://api.openai.com/v1/responses', {
      ...
      body: JSON.stringify({ model, max_output_tokens: maxTokens, input: [ ...(system ? [{ role: 'system', content: [{ type: 'input_text', text: system }] }] : []), { role: 'user', content: [{ type: 'input_text', text: prompt }] } ] }),
    })
    const data = await res.json()
    if (!res.ok) throw new Error(data.error?.message || `OpenAI error ${res.status}`)
    if (typeof data.output_text === 'string') return data.output_text
    return data.output?.flatMap((item: any) => item.content || [])?.map((c: any) => c.text || '').join('') || ''
  }
...
async function callOpenAICompat(prompt: string, system: string | undefined, maxTokens: number, apiKey: string, model: string, baseUrl: string): Promise<string> {
  const res = await fetch(`${baseUrl}/chat/completions`, { method: 'POST', headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${apiKey}` }, body: JSON.stringify({ model, max_tokens: maxTokens, messages: [ ...(system ? [{ role: 'system', content: system }] : []), { role: 'user', content: prompt } ] }) })
  const data = await res.json()
...
async function callOpenRouter(prompt: string, system: string | undefined, maxTokens: number, apiKey: string, model: string): Promise<string> {
  const res = await fetch('https://openrouter.ai/api/v1/chat/completions', { method: 'POST', headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${apiKey}`, 'HTTP-Referer': process.env.NEXT_PUBLIC_APP_URL || 'https://gabaritoia.com', 'X-Title': 'GabaritoIA' }, body: JSON.stringify({ model, max_tokens: maxTokens, messages: [ ...(system ? [{ role: 'system', content: system }] : []), { role: 'user', content: prompt } ] }) })
  const data = await res.json()
...
function getEnvKey(provider: AIProvider): string | undefined {
  const map: Record<AIProvider, string | undefined> = { claude: process.env.ANTHROPIC_API_KEY, openai: process.env.OPENAI_API_KEY, gemini: process.env.GEMINI_API_KEY, grok: process.env.GROK_API_KEY, openrouter: process.env.OPENROUTER_API_KEY }
  return map[provider]
}

function getDefaultModel(provider: AIProvider): string {
  const map: Record<AIProvider, string> = { claude: 'claude-sonnet-4-20250514', openai: 'gpt-5.5', gemini: 'gemini-2.0-flash', grok: 'grok-2', openrouter: 'google/gemini-2.0-flash-001' }
  return map[provider]
}

function sanitizeJsonLike(raw: string): string {
  return raw.trim().replace(/^```json\s*/i, '').replace(/^```\s*/i, '').replace(/```$/i, '').replace(/[\u0000-\u001F\u007F]/g, ch => (ch === '\n' || ch === '\r' || ch === '\t' ? ch : '')).replace(/,\s*([}\]])/g, '$1').trim()
}

function extractBalancedJson(text: string): string | null {
  for (const candidate of [{ char: '{', close: '}' }, { char: '[', close: ']' }]) {
...
export function parseAIJson<T = unknown>(raw: string): T {
  ...
  for (const attempt of attempts) {
    try { return JSON.parse(sanitizeJsonLike(attempt)) as T } catch {}
  }
